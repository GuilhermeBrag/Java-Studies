...
//          Map <Estado,Cidade> capitais = new TreeMap<>();
        ...
        for (var estadoCidadeEntry : capitais.entrySet()) {
            System.out.printf("Cód.IBGE= %d | Cidade = %s | Estado = %s | Total Habitantes = %d %n",
                    estadoCidadeEntry.getValue().getCodigoIbge(), estadoCidadeEntry.getKey().getNome(),
                    estadoCidadeEntry.getKey().getSigla(), estadoCidadeEntry.getValue().getTotalHabitantes());
        }
    }
